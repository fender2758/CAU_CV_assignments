...
class fretDetector():
    ...
    def fretDetection_board(self, source, edge_threshold=20):
        ...
        cv2.imshow("neck_str_gray", neck_str_gray)
        
        # 2. Slice image vertically at different points and calculate gaps between strings at these slices
        slices = {}
        split = 20
        nb_slices = int(height / split)
        for i in range(nb_slices):
            slices[(i + 1) * split] = []  # slices dict is {x_pixel_of_slice : [y_pixels_where_line_detected]}
        ...
        for index_line in range(split, len(edges), split):
            index=0
            while index < len(edges[0]):
                if edges[index_line][index] == 255:
                    temp = index
                    while index < len(edges[0]) and edges[index_line][index] == 255:
                        index+=1
        ...
        #cv2.imshow("fret_npmargin", image[x1:x2][y1:y2])
        y1_min, y2_min = int((y1_min+y2_min)/2), int((y1_min+y2_min)/2)
        y1_max, y2_max = int((y1_max+y2_max)/2), int((y1_max+y2_max)/2)
        y1_min-=margin1
        y2_min-=margin2
        y1_max+=margin1
        y2_max+=margin2
