...
class fretDetector():
    def __init__(self, fps, width, height):
        ...
        self.width = width
        self.height = height
        self.fret_buff =np.full((int(fps), 4, 2), -1, dtype=np.float32)
        self.board_buff =np.full((int(fps), 16, 2), -1, dtype=np.float32)
        
        self.fret = None
        self.frets = None
        self.board = None
        self.tab_points = None
        self.M = None
        
        first_fret = 100
        self.print_frets = [0, first_fret]
        for i in range(9):
            self.print_frets.append(int((self.print_frets[-1]-self.print_frets[-2])*16.817/17.817+self.print_frets[-1]))
        
    def fretDetection(self, source, top, bottom, edge_threshold=20):
        ...
        #print(edges.shape)
        edges__ = image.copy()
        edges_ = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
        edges__ = np.vstack((edges__, edges_))
        #print(edges.shape)
        ...
        cv2.imshow("edges horizontal", edges__)
        ...
        tun_color = [(255, 0, 0), (0, 0, 255), (255, 255, 255), (0, 0, 0), (0, 255, 255), (255, 255, 0)]
        points_divided = [[] for i in range(6)]
        ...
        margin1=int((y1_max-y1_min)/10)
        margin2=int((y2_max-y2_min)/10)
        ...
        #print(pts1)
        self.fret_buff[:-1] = self.fret_buff[1:]
        self.fret_buff[-1] = pts1
        ...
        self.fret = fret
        return src, fret, neck_with_frets, M
    

    def fretDetection_board(self, source, edge_threshold=20):
        image = source.copy()
        ...
        gray = cv2.filter2D(gray, -1, kernel_sharpening)
        edges__ = image.copy()
        edges = cv2.Sobel(gray, cv2.CV_8U, 2, 0, 3)
        edges_ = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
        edges__ = np.vstack((edges__, edges_))
        #print(edges.shape)
        ...
        kernel = np.ones((height*2, 1), np.uint8)
        edges = cv2.dilate(edges, kernel, iterations = 1)
        edges_ = cv2.cvtColor(edges, cv2.COLOR_GRAY2BGR)
        edges__ = np.vstack((edges__, edges_))
        
        cv2.imshow("edges vertical", edges__)
        
        #edges = cv2.GaussianBlur(edges, (0, 0), )
        #print(edges)
        # edges = cv2.medianBlur(edges, 3)

        lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 20, 50, 4)
        ...
        index = width-1
        while index >=0:
            if edges[index_line][index] == 255:
                temp = index
                while index >=0 and edges[index_line][index] == 255:
                    index-=1
                test_neck[index_line][int((index+temp)/2)]=255
                slices.append(int((index+temp)/2))
            index-=1
        ...
        for i in range(len(slices)-2):
            num_fret = 1
            temp_slice = slices[i+1]-slices[i+2]
            
            while abs(self.get_length(temp_slice, num_fret)-(slices[i]-slices[i+1]))>10:
                #print(self.get_length(temp_slice, num_fret), (slices[i]-slices[i+1]), num_fret)
                if num_fret>10:
                    return 
                num_fret+=1
            frets.append(int(slices[i]))
            if len(frets)==10:
                break
            temp_fret = slices[i+2]
            temp_frets = []
            for i in range(num_fret):
                temp_frets.append(int(temp_fret+temp_slice))
                if len(frets)==10:
                    break
                
                temp_fret+=temp_slice
                temp_slice*=17.817/16.817
                
            frets+=temp_frets[1:][::-1]
            if len(frets)==10:
                break
        self.frets = frets
    ...
    def print_fret(self):
        if self.frets:
            temp_fret = self.fret.copy()
            temp_fret = temp_fret[:, self.frets[-1]: self.frets[0]]
            for f in  self.frets:
                cv2.line(temp_fret, (int(f- self.frets[-1]), 0), (int(f- self.frets[-1]), int(self.fret.shape[0]-1)), (0, 0, 255), 2)
                cv2.line(self.fret, (int(f), 0), (int(f), int(self.fret.shape[0]-1)), (0, 0, 255), 2)
            for point in self.tab_points:
                
                cv2.circle(temp_fret, (int(point[0]-self.frets[0]), int(point[1])), 3, (0, 0, 255), -1)
                cv2.circle(self.fret, (int(point[0]), int(point[1])), 3, (0, 0, 255), -1)
                

            cv2.imshow("temp_fret", temp_fret)
            cv2.imshow("fret", self.fret)
        return
    
    def get_fingering(self):
        fingers = []
        height = self.fret.shape[0]
        for finger in self.tab_points:
            line_num = int((finger[1]/height)*6)
            line_num = -1 if line_num>5 or line_num<0 else line_num
            fret_num=0
            while line_num!=-1 and fret_num<len(self.frets) and self.frets[fret_num]>finger[0]:
                fret_num+=1
            fingers.append([line_num, fret_num])
        return fingers
    
    def print_fingering(self, fingers=None):
        board = np.zeros((100, self.print_frets[-1], 3))
        font =  cv2.FONT_HERSHEY_PLAIN
        for i in range(11):
            for f in self.print_frets:
                cv2.line(board, (int(f), 0), (int(f), 99), (255, 255, 255), 2)
        for i in range(6):
            for f in self.print_frets:
                cv2.line(board, (0, 5+18*i), (self.print_frets[-1], 5+18*i), (255, 255, 255), 2)
        if fingers:
            for i, (l, f) in enumerate(fingers):
                if f!=0:
                    fr = int((self.print_frets[f]+self.print_frets[f-1])/2)
                    li = 5+18*(5-l)
                    cv2.circle(board, (fr, li), 5, (0,0,0),-1)
                    board = cv2.putText(board, str(i), (fr-5, li+4), font, 1, (255, 255, 255), 1, cv2.LINE_AA)
        return board
        
    def update_tabs(self, tab_points):
        """
    ...
    def get_length(self, fret, num_fret):
        length = 0
        for i in range(num_fret):
            fret*=17.817/16.817
            length+=fret
